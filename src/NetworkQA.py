...
def get_hous(s):
    if not 0 < int(s[:-2]) < 24:
        ...
        send_msg('hous must in [0,24],please include a correct time!')
        return False
    if 'am' in s:
        if int(s[:-2]) > 12:
            print('am hous must in [0,12]')
            send_msg('am hous must in [0,12],please include a correct time!')
            return False
...
def weather(text):
    if ('am' not in text) and ('pm' not in text):
        ...
        while True:
            send_msg('place input am or pm')
            x = recv_meg().lower()
            if x == 'am' or x == 'pm':
                text = text+x
                break

            ##send_msg('only am or pm,retry!')
    ...
    ave = {v: k for k, v in key2.items()}[times] + ' {} '.format(h) + c + ' is ' + str(result) + ', average :'+c + ':'+ str(avg)
    extreme=f'max {c}:{mx},min {c}:{mn}'
    print(f'max {c}:{mx},min {c}:{mn}')
    ##da = 'now date:', now_datetime.strftime('%Y-%m-%d')
    ##send_msg(da)
    send_msg(ave+extreme)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        try:
            ...
            send_msg('Please re input the question')
            print('re input')